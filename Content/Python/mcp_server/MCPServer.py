"""
MCPServer - 编辑器内MCP服务器

在Unreal编辑器进程内运行的MCP服务器
"""
...
class MCPServer:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 8099):
        ...
        if MCP_AVAILABLE and STARLETTE_AVAILABLE:
        ...
        try:
            ...
            async def call_tools(name: str, arguments: dict) -> list:
                ...
                if name == "execute_command":
                    result = CodeExecutor.execute_code(arguments.get('code', ''))
                    return result.to_mcp_content()
                    
                elif name == "excute_file":
                    result = CodeExecutor.execute_file(arguments.get('file', ''))
                    return result.to_mcp_content()
